...
# Authentication
JWT_SECRET=${generateRandomStrings(1, 32)[0]}
TOKEN_EXPIRY=7d
...
VITE_API_URL=http://localhost:4000/api${
    projectConfig.backend?.apiVersioning ? "/v1" : ""
  }
...
`;
  ...
  const backendEnvCreated = createProjectFile(
    "backend/.env",
    backendEnvContent
  );
  const frontendEnvCreated = createProjectFile(
    "frontend/.env",
    frontendEnvContent
  );

  return backendEnvCreated && frontendEnvCreated;
}
...
function generateWebAppStructure(): boolean {
  ...
  const frontendDirCreated = fileOperation(
    () => {
      fs.mkdirSync(path.join(projectConfig.outputDir!, "frontend"), {
        recursive: true,
      });
      console.log(chalk.green(`✓ Created directory: frontend/`));
      return true;
    },
    `Would create directory: frontend/`,
    `Failed to create directory: frontend/`
  );

  if (frontendDirCreated === null && !options.dryRun) {
    success = false;
  }

  // Define the folders to create inside frontend directory
  const folders = [
    "public",
    "src/app",
    "src/components",
  ...
  // Create each folder within frontend directory
  for (const folder of folders) {
    const created = fileOperation(
      () => {
        fs.mkdirSync(path.join(projectConfig.outputDir!, "frontend", folder), {
          recursive: true,
        });
        console.log(chalk.green(`✓ Created directory: frontend/${folder}/`));
        return true;
      },
      `Would create directory: frontend/${folder}/`,
      `Failed to create directory: frontend/${folder}/`
    );
  ...
  // Initialize React + TypeScript app base files
  const reactTsInitialized = initializeReactTypeScriptApp();
  if (!reactTsInitialized && !options.dryRun) {
    console.log(
      chalk.yellow(
        "⚠️ Warning: React + TypeScript files could not be fully generated"
      )
    );
    success = false;
  }
...
/**
 * Initialize a minimal React + TypeScript application
 */
function initializeReactTypeScriptApp(): boolean {
  console.log(
    chalk.blue("\n🔧 Initializing React + TypeScript application...")
  );
  let success = true;

  // Generate package.json with React and TypeScript dependencies
  const packageJsonCreated = generateReactPackageJson();
  if (!packageJsonCreated && !options.dryRun) {
    success = false;
  }

  // Generate tsconfig.json for TypeScript configuration
  const tsconfigCreated = generateTypeScriptConfig();
  if (!tsconfigCreated && !options.dryRun) {
    success = false;
  }

  // Generate index.html template
  const htmlCreated = generateIndexHtml();
  if (!htmlCreated && !options.dryRun) {
    success = false;
  }

  // Generate vite.config.ts
  const viteConfigCreated = generateViteConfig();
  if (!viteConfigCreated && !options.dryRun) {
    success = false;
  }
 ...
 */
function generateReactPackageJson(): boolean {
  console.log(chalk.blue("📄 Generating frontend/package.json..."));
  ...
  // Create the package.json file in frontend directory
 ...
 * Generate tsconfig.json for TypeScript configuration
 */
function generateTypeScriptConfig(): boolean {
  console.log(chalk.blue("📄 Generating frontend/tsconfig.json..."));
  ...
  // Create tsconfig.node.json as well
  const tsNodeConfigContent = {
    compilerOptions: {
      composite: true,
      skipLibCheck: true,
      module: "ESNext",
  ...
  // Create the tsconfig.json file in frontend directory
  const mainConfigCreated = createProjectFile(
    "frontend/tsconfig.json",
    JSON.stringify(tsConfigContent, null, 2)
  );

  // Create the tsconfig.node.json file in frontend directory
  const nodeConfigCreated = createProjectFile(
    "frontend/tsconfig.node.json",
    JSON.stringify(tsNodeConfigContent, null, 2)
  );

  return mainConfigCreated && nodeConfigCreated;
}

/**
 * Generate vite.config.ts file for build configuration
 */
function generateViteConfig(): boolean {
  console.log(chalk.blue("📄 Generating frontend/vite.config.ts..."));
...
`;
  ...
  // Create the vite.config.ts file in frontend directory
  return createProjectFile("frontend/vite.config.ts", configContent);
}

/**
 * Generate index.html template
 */
function generateIndexHtml(): boolean {
  console.log(chalk.blue("📄 Generating frontend/index.html..."));
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="${
      projectConfig.projectName || "React TypeScript App"
    } created with CLI Project Generator" />
    <title>${projectConfig.projectName || "React TypeScript App"}</title>
  </head>
  <body>
    <div id="root"></div>
...
`;
  ...
  // Create the index.html file in frontend directory
  return createProjectFile("frontend/index.html", htmlContent);
}
...
function generateWebIndexFile(): boolean {
  console.log(chalk.blue("📄 Generating starter frontend/src/index.tsx..."));
      ...
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded text-sm">
          ...
          <ul className="list-disc pl-5 text-blue-700">
            ...
            <li>Add shared components in src/components</li>
            <li>Happy coding!</li>
          </ul>
        </div>
      </div>
...
function generateBackendStructure(): boolean {
  ...
  const backendDirCreated = fileOperation(
    () => {
      fs.mkdirSync(path.join(projectConfig.outputDir!, "backend"), {
        recursive: true,
      });
      console.log(chalk.green(`✓ Created directory: backend/`));
      return true;
    },
    `Would create directory: backend/`,
    `Failed to create directory: backend/`
  );

  if (backendDirCreated === null && !options.dryRun) {
    success = false;
  }

  // Define the folders to create inside backend directory
  const folders = [
    "src/config",
    "src/controllers",
    "src/middleware",
  ...
  // Create each folder within backend directory
  for (const folder of folders) {
    const created = fileOperation(
      () => {
        fs.mkdirSync(path.join(projectConfig.outputDir!, "backend", folder), {
          recursive: true,
        });
        console.log(chalk.green(`✓ Created directory: backend/${folder}/`));
        return true;
      },
      `Would create directory: backend/${folder}/`,
      `Failed to create directory: backend/${folder}/`
    );
  ...
  // Create a starter index.ts file and package.json
  const indexCreated = generateBackendIndexFile();
  const packageJsonCreated = generateBackendPackageJson();

  if ((!indexCreated || !packageJsonCreated) && !options.dryRun) {
    success = false;
  }
 ...
 */
function generateBackendPackageJson(): boolean {
  console.log(chalk.blue("📄 Generating backend/package.json..."));

  const packageContent = {
    name: projectConfig.projectName
      ? `${projectConfig.projectName}-backend`
      : "express-backend",
    version: "0.1.0",
    private: true,
    main: "dist/index.js",
    ...
    dependencies: {
      ...
      ...(projectConfig.backend?.database === "mongodb"
        ? { mongoose: "^7.0.3" }
        : { pg: "^8.10.0" }),
      ...(projectConfig.backend?.auth?.jwt ? { jsonwebtoken: "^9.0.0" } : {}),
    },
    devDependencies: {
      "@types/cors": "^2.8.13",
  ...
  // Create the package.json file in backend directory
  return createProjectFile(
...
function generateBackendIndexFile(): boolean {
  console.log(chalk.blue("📄 Generating starter backend/src/index.ts..."));
...
`;
  ...
  // Create the index.ts file in backend/src directory
  const indexCreated = createProjectFile("backend/src/index.ts", indexContent);

  // Create the tsconfig.json file in backend directory
  const tsconfigCreated = createProjectFile(
    "backend/tsconfig.json",
    JSON.stringify(tsConfigContent, null, 2)
  );

  return indexCreated && tsconfigCreated;
}
